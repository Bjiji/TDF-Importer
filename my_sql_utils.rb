...
class MySQLUtils
  ...
  def self.updateTeamLabelAndCode(race_team, team_code, team_label)
    team_id = race_team['team_id']
    team = @@client.query("update teams set uci = '#{team_code}', name = '#{team_label}' where id = '#{team_id}'")
  end
  ...
  def self.updateCyclist(cyclist_id, dob)
    @@client.query("update cyclists set dob = '#{dob}' where id = #{cyclist_id}")
  end
...
#     cyclist_id = @@client.query("SELECT distinct c.*, min(rr.year) 'min_year', max(rr.year) as 'max_year' FROM cyclists c join race_runners rr on rr.cyclist_id = c.id WHERE c.lastname LIKE '#{mescape(lastname)}' AND c.firstname LIKE '#{firstname.chars.first}%' group by c.id HAVING (#{year} < max_year + 5 AND #{year} > min_year - 5)")
  ...
  def self.getOrCreateRaceRunnerASO(year, dossard, lastname, firstname, nationality, team_code, team_name, dob)
    ...
    if (team == nil) then
      team = getRaceTeam(team_name, year)
      if (team == nil) then
        raise "Unable to find team with code '#{team_code}' or team_name '#{team_name}' for year #{year}. Please create it first"
      else
        updateTeamLabelAndCode(team, team_code, team_name)
      end
    end
    race_team_id = team['id']
    raceRunner = getRaceRunner(year, cyclist_id, race_team_id)
